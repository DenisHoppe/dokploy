...
export const MoveApplicationToServer = ({ applicationId }: Props) => {
	...
	const [pendingDeploymentId, setPendingDeploymentId] = useState<string>();
	...
	const effectivePendingDeploymentId =
		pendingDeploymentId ?? pendingMove?.deploymentId;
	...
	const handleFinalize = async () => {
		if (!effectivePendingDeploymentId) return;
		await finalize.mutateAsync({
			applicationId,
			deploymentId: effectivePendingDeploymentId,
		});
	...
	return (
		<Dialog
			...
			onOpenChange={(nextOpen) => {
				setOpen(nextOpen);
				if (!nextOpen && !effectivePendingDeploymentId) {
					setTargetServerId("");
				}
			}}
		>
			...
			<DialogContent className="sm:max-w-lg">
				...
				{!effectivePendingDeploymentId ? (
					...
					<AlertBlock type="warning">
						The target deployment succeeded. Confirm that DNS and health checks
						point to the target before cleaning up the source server.
					</AlertBlock>
				)}
