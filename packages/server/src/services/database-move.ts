...
export type DatabaseMoveServiceType = Exclude<MoveableServiceType, "compose">;
...
interface DatabaseMoveAdapter {
	...
	deploy: (id: string, onData?: (data: any) => void) => Promise<unknown>;
}
...
export const moveDatabaseToServer = async ({
	...
	try {
		...
		// Re-check for a name collision immediately before deploying, not just
		// once at the very start of this function: the volume/file transfer
		// above can take a long time, during which another process could have
		// created a same-name service on the target. Re-confirming absence
		// here shrinks (it cannot fully eliminate) that TOCTOU window, and -
		// combined with `targetServiceCreated` only flipping true after
		// `adapter.deploy` returns without throwing - means rollback can only
		// ever remove a service this migration itself is responsible for.
		...
		if (collisionAppeared) {
			throw new Error(
				`A Docker service named "${entity.appName}" appeared on the target server while data was being transferred. Refusing to deploy onto it - this migration cannot safely establish ownership.`,
			);
		}

		await adapter.deploy(id);
...
export const getPendingDatabaseMove = async ({
	...
	const migration = await findPendingServiceMigration(serviceType, id);
...
export const finalizeDatabaseMove = async ({
	...
	for (const mount of entity.mounts.filter(
		(mount) => mount.type === "volume",
	)) {
		if (mount.volumeName) {
			await removeVolumeIdempotent(mount.volumeName, migration.sourceServerId);
		}
	}
